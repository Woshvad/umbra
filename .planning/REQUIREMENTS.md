...
- [x] **WOW-06**: A cost-of-leakage simulator runs the same orders through a simulated public order book (front-run/slippage → $ lost) beside Umbra's sealed clear ($0 leaked)
...
- [x] **AUCT-01**: Desks can submit richer order types — noncompetitive ("fill at clear"), minimum-acceptable-quantity / all-or-none, and conditional auto-firming — in addition to plain sealed limits *(09-06; OrderTicket LIMIT·NONCOMP·MAQ·COND selector + per-type params + type-aware Venue.SubmitOrder on the desk's own token — web build + 30 vitest green; on-ledger clearing math shipped 09-02/09-03; live per-type submit against a running open round = end-of-phase human-check)*
