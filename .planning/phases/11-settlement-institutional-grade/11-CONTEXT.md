...
- **Library-first, faithful-fallback:** PREFER the real Daml Finance packages (`daml-finance-interface-*` + `daml-finance-holding`, `-account`, `-instrument-token`, `-settlement`) pinned to the release that builds on **SDK 3.4.11** (the phase-researcher CONFIRMS the compatible Daml Finance version + data-dependency wiring before planning commits). If — and only if — the real library cannot be made to build/vet on the 3.4.11 line within the phase, fall back to a **faithful in-repo implementation of the SAME interfaces** (Account / Holding / Instrument + Batch / Instruction `allocate → approve → settle` with explicit finality), honestly labeled as a Daml-Finance-pattern layer rather than the library. Attempt real first; prefer a smaller real integration over a larger fake one (the Phase-10 honesty bar).
