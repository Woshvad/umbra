...
- **`self` payer authentication (CR-01 fix):** because the `self` fee move runs under operator authority (no on-ledger payer signature), the gate now AUTHENTICATES the caller — it requires the desk's own party token (the same dev-HS256 / OIDC token the JSON API uses; verified in `payer-auth.ts`, no new dependency) and binds the fee source to `owner === authenticatedParty`. A forged/unauthenticated `from` is rejected `unauthorized_payer` and no funds move. The subject→party map is built from the deploy party map (`umbra-<hint>` convention) + the operator user, overridable via `X402_PAYER_PARTY_MAP`; unknown subjects fail closed. **UAT:** confirm each desk presents `Authorization: Bearer <its party token>` on the metered call, and that a spoofed `from` with no/other token is refused. The stronger **payer-signed on-ledger transfer remains the deferred `canton-cc` variant** (Gate 3).
